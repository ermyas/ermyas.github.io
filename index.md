---
## You don't need to edit this file, it's empty on purpose.
## Edit theme's home layout instead if you wanna make some changes
## See: https://jekyllrb.com/docs/themes/##overriding-theme-defaults
layout: default
---
Ermyas Abebe
===
**Research Manager at IBM Research, Australia**

Education
====
-----------------------

## Ph.D. in Computer Science
  *[RMIT University](https://www.rmit.edu.au/)*

  - **Thesis:** [Improving the Efficiency and Efficacy of Adaptive Computation Offloading in Pervasive Computing Environments](https://researchbank.rmit.edu.au/view/rmit:160267)
  - **Award:** Letter of Recognition for Outstanding Thesis Assessment

## B.Sc. in Computer Science
  *[RMIT University](https://www.rmit.edu.au/)*

  - **GPA:** 4.0
  - **Award:** Highest Distinction *(Summa Cum Laude)*

Experience
====
-----------------------

## IBM Research [2013 - Current]

**Manager Research Scientist - Blockchain Solutions**

  - 2015 – Today: Currently leading a team of researchers who are focused on developing a number of innovative Blockchain based solutions for the financial services sector.
  - 2014 – 2015: Led a team of researchers and software engineers towards designing and developing a number of novel research prototypes in the healthcare, life-sciences, and financial services spaces.
  - 2013 – 2014: Led a team of researchers and software engineers towards designing, and developing a number of systems for modelling and predicting natural disasters and population evacuation scenarios.

## RMIT University [Jun. 2008 - Feb 2013]
**Sessional Lecturer & Head Teaching Assistant** *(part-time)*

## PerSys Solutions [Aug. 2012 - Feb 2013]
**Lead Software Engineer**

## Komposite Systems [Jul. 2007 - Mar. 2008]
**Lead Software Engineer** *(part-time)*

## Amest Santim Systems [Dec. 2006 - Mar. 2008]
**Software Engineer** *(part-time)*

## Cybersoft Systems [Jul. 2006 - Jan. 2007]
**Junior Developer** *(part-time)*

Interests
===
-----------------------

- Blockchain/Distributed Ledger Technologies

- Software Design and Architecture

- Distributed Systems

- Algorithms and Datastructures

- Functional programming

- APIs and Webservices

- Cloud computing platforms

Skills
===
-----------------------

- *Programming Languages:* Java, Scala, C, Python, Typescript, Javascript, [Haskell]

- *Frameworks:* Scalaz, Apache Thrift, Java-EE, Meteor.js, Android SDK

- *Tools:*  Docker, Travis, Jenkins

- *Processes and Practices:* Agile Development, Design Thinking, DevOps, Continuous Integration and Deployment, TDD, BDD

- *Architecture:* Micro-services, Evolutionary Architectures, Domain Driven Design, Lambda Architectures, Serverless, CQRS, Event Driven Architectures

- *Databases:* Relational (Postgres, MySQL, SQLite) and NoSQL (CouchDb, MongoDB)

- *Cloud Computing:* Amazon EC2, Bluemix, Softlayer, Google AppEngine

- *Big Data:* Hadoop, Spark


Awards
====
-----------------------

- IBM A-Level Accomplishment Award for business impact through Client Engagement

- IBM A-Level Accomplishment Award for business impact through innovation in tools and process

- IBM Outstanding Technical Achievement Award

- IBM Research Division Award

- IBM Open Source Award For Significant Opensource Contributions to CouchDB and Cloudant

- Third-prize winner of Startup Weekend Melbourne

- First-prize winner of IBM MobileFirst Hackathon

- Letter of Recognition for Outstanding PHD Thesis Assessment result

- Third Pace in Melbourne Startup Weekend competition

- RMIT University International Research Student Scholarship (RUIRS) (Full Scholarship for PhD) 2008 - 2012

- RMIT University Graduation with Highest Distinction Award

- Best Presentation Award – RMIT University Computer Science Research Students’ Conference 2010

- Best Paper Award – RMIT University Computer Science Research Students’ Conference 2009

- Highest Cumulative GPA Award - RMIT University 2007


Open source
====
-----------------------

- [CouchDB](https://github.com/beloglazov/couchdb-scala): Purely functional Scala client for CouchDB.

- [Web3J](https://github.com/web3j/web3j): Lightweight Java and Android library for integration with Ethereum clients

- [Trek](https://github.com/ermyas/trek): A geospatial puzzle game where you guess the site in the world that corresponds to a given clue

- [Quorum](https://github.com/jpmorganchase/quorum): A permissioned implementation of Ethereum supporting data privacy

Patents
====
-----------------------

- Filed Patents: 20

- Issued Patents: 4

Publications
====
-----------------------

- Zhang, W. E., Sheng, Q. Z., Lau, J. H., & **Abebe, E.** (2017, April). Detecting Duplicate Posts in Programming QA Communities via Latent Semantics and Association Rules. In Proceedings of the 26th International Conference on World Wide Web (pp. 1221-1229). International World Wide Web Conferences Steering Committee.

- Khan, K. H., Ryan, C., & **Abebe, E.** (2017). Optimizing HVAC Energy Usage in Industrial Processes by Scheduling Based on Weather Data. IEEE Access, 5, 11228-11235.

- Zhang, W. E., **Abebe, E.**, Sheng, Q. Z., & Taylor, K. L. (2016). Towards Building Open Knowledge Base From Programming Question-Answering Communities. In International Semantic Web Conference (Posters & Demos).

- Zhang, W. E., Sheng, Q. Z., **Abebe, E.**, Babar, M. A., & Zhou, A. (2016). Mining Source Code Topics Through Topic Model and Words Embedding. In Advanced Data Mining and Applications: 12th International Conference, ADMA 2016, Gold Coast, QLD, Australia, December 12-15, 2016, Proceedings 12 (pp. 664-676). Springer International Publishing.

- Beloglazov, A., Almashor, M., **Abebe, E.**, Richter, J., & Steer, K. C. B. (2016). Simulation of wildfire evacuation with dynamic factors and model composition. Simulation Modelling Practice and Theory, 60, 144-159.

- Adem, K., Ryan, C., & **Abebe, E.** (2015, January). Crowdsourcing the cloud: Energy-aware computational offloading for pervasive community-based cloud computing. In Proceedings of the International Conference on Parallel and Distributed Processing Techniques and Applications (PDPTA) (p. 415). The Steering Committee of The World Congress in Computer Science, Computer Engineering and Applied Computing (WorldComp).

- **Abebe, E.**, & Ryan, C. (2012). Adaptive application offloading using distributed abstract class graphs in mobile environments. Journal of Systems and Software, 85(12), 2755-2769.

- **Abebe, E.**, & Ryan, C. (2011, August). A hybrid granularity graph for improving adaptive application partitioning efficacy in mobile computing environments. In Network Computing and Applications (NCA), 2011 10th IEEE International Symposium on (pp. 59-66). IEEE.

- **Abebe, E.**, & Ryan, C. (2011, August). Improving adaptive offloading using distributed abstract class graphs in mobile environments. In Network Computing and Applications (NCA), 2011 10th IEEE International Symposium on (pp. 257-264). IEEE.

- **Abebe, E.**, & Ryan, C. (2009). A distributed approach to local adaptation decision making for sequential applications in pervasive environments. On the Move to Meaningful Internet Systems: OTM 2009, 773-789.


Misc. Projects
====
-----------------------

- [Jember Calendar Mobile  App](https://play.google.com/store/apps/details?id=com.lucy.ethiopian.calendar&hl=en) (2013)

- PhD Student Co-supervisor at RMIT University (2012-*current*)

- RMIT University Research Committee Student Representative (2010-2011)

- Business Process Re-engineering Software - Maritime and Transit Service Enterprise, Ethiopia (2007)

- Web-based Thesis Library – Addis Ababa University (2006)

- RMIT University - Addis Ababa University Student Council President (2005-2006)
